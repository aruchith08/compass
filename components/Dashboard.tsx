...
import { Target, Trophy, Flame, ListTodo, BookOpen, Plus, Trash2, CheckCircle, Presentation, Search, ExternalLink, Quote, GraduationCap, CalendarClock, PartyPopper, Sparkles } from 'lucide-react';
  ...
  useEffect(() => {
    if (active) {
      ...
      const newParticles = Array.from({ length: count }).map((_, i) => {
        ...
        const xMid = (Math.random() - 0.5) * 500; // Wide horizontal spread
        const yMid = - (Math.random() * 400 + 150); // Strong upward push
        ...
        return {
          ...
          delay: Math.random() * 0.4, // Fast staggered start
          vars: {
            '--x-mid': `${xMid}px`,
            '--y-mid': `${yMid}px`,
            '--x-end': `${xEnd}px`,
            '--rotation': `${Math.random() * 1440}deg`,
          }
        };
      });
  ...
  return (
    <div className="absolute inset-x-0 bottom-0 top-0 pointer-events-none z-50 overflow-hidden">
      {particles.map(p => (
        <div 
          key={p.id}
          className="absolute left-1/2 bottom-0 animate-confetti-burst"
          style={{ 
            backgroundColor: p.color, 
            width: p.isCircle ? `${p.size}px` : `${p.size * 1.8}px`, 
            height: `${p.size}px`, 
...
const Dashboard: React.FC = () => {
  ...
  // Refs to prevent double trigger during same render
  const dailyWasDoneRef = useRef(allDailyDone);
  const hwWasDoneRef = useRef(allHomeworkDone);

  useEffect(() => {
    if (allDailyDone && !dailyWasDoneRef.current) {
      setCelebrateDaily(true);
      const t = setTimeout(() => setCelebrateDaily(false), 5000);
      dailyWasDoneRef.current = true;
  ...
  useEffect(() => {
    if (allHomeworkDone && !hwWasDoneRef.current) {
      setCelebrateHomework(true);
      const t = setTimeout(() => setCelebrateHomework(false), 5000);
      hwWasDoneRef.current = true;
      return () => clearTimeout(t);
    }
  ...
  const roleData = roles.map(role => ({
    name: role.replace("Officer", "").replace("Engineer", "Eng").replace("Developer", "Dev").replace("Scientist", "Sci"),
    fullName: role,
  ...
  const nextFocusItems = items
    .filter(i => (i.status === 'To Do' || i.status === 'In Progress') && i.priority === 'High')
    .slice(0, 3);
  ...
  return (
    <div className="space-y-8 pb-12 w-full max-w-full overflow-hidden">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 w-full animate-slide-up" style={{ animationDelay: '100ms' }}>
        ...
        <div className={`${glassCardClass} flex flex-col h-full min-h-[300px] relative transition-all duration-500 overflow-hidden ${allDailyDone ? 'border-amber-400 shadow-amber-200/40' : 'hover:shadow-xl hover:shadow-indigo-200/50 hover:border-indigo-200'}`}>
          ...
          <div className="flex items-center justify-between mb-4 relative z-10">
            ...
            {allDailyDone ? (
              <span className="text-[10px] font-bold text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-900/30 px-2 py-1 rounded-full animate-bounce flex items-center gap-1">
                <Sparkles size={10} /> DAY CLEAR <Sparkles size={10} />
              </span>
            ) : (
          ...
          <div className="flex-1 space-y-2 mb-4 overflow-y-auto max-h-[200px] custom-scrollbar relative z-10">
             {dailyTasks.map(task => (
               <div key={task.id} className="flex items-start group">
                 <button 
                   ...
                   className={`mt-0.5 mr-3 flex-shrink-0 w-5 h-5 rounded-md border flex items-center justify-center transition-all duration-200 cursor-pointer ${
                     task.completed 
                       ...
                       : 'bg-slate-100/50 dark:bg-slate-800/80 border-slate-300 dark:border-slate-500 hover:border-indigo-400 hover:bg-white'
                   }`}
                 >
                 ...
                 {!task.isFixed && (
                   <button onClick={() => deleteDailyTask(task.id)} className="opacity-0 group-hover:opacity-100 p-1 text-slate-400 hover:text-rose-500 transition-opacity"><Trash2 size={14} /></button>
                 )}
               </div>
             ))}
          </div>
          
          <form onSubmit={handleAddDaily} className="relative mt-auto z-10">
             <input 
               type="text" value={newDaily} onChange={(e) => setNewDaily(e.target.value)} placeholder="Add routine task..."
               className="w-full bg-slate-50/80 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700/50 rounded-lg py-2 pl-3 pr-10 text-sm focus:outline-none focus:border-indigo-500 dark:text-white placeholder-slate-400"
             />
             <button type="submit" className="absolute right-2 top-1/2 -translate-y-1/2 p-1 bg-indigo-500 hover:bg-indigo-600 text-white rounded-md transition-colors"><Plus size={14} /></button>
          </form>

          {/* Sparkle Decoration for Complete State */}
          {allDailyDone && (
            <div className="absolute top-0 right-0 p-4 opacity-10 pointer-events-none animate-sparkle">
        ...
        <div className={`${glassCardClass} flex flex-col h-full min-h-[300px] relative transition-all duration-500 overflow-hidden ${allHomeworkDone ? 'border-emerald-400 shadow-emerald-200/40' : 'hover:shadow-xl hover:shadow-rose-200/50 hover:border-rose-200'}`}>
          ...
          <div className="flex items-center justify-between mb-4 relative z-10">
            ...
            {allHomeworkDone ? (
              <span className="text-[10px] font-bold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/30 px-2 py-1 rounded-full animate-bounce flex items-center gap-1">
                <Sparkles size={10} /> MISSION COMPLETE <Sparkles size={10} />
              </span>
            ) : (
          ...
          <div className="flex-1 space-y-2 mb-4 overflow-y-auto max-h-[200px] custom-scrollbar relative z-10">
             ...
             {homeworkTasks.map(task => (
               <div key={task.id} className="flex items-start group">
                 <button 
                   ...
                   className={`mt-0.5 mr-3 flex-shrink-0 w-5 h-5 rounded-md border flex items-center justify-center transition-all duration-200 cursor-pointer ${
                     task.completed 
                       ...
                       : 'bg-slate-100/50 dark:bg-slate-800/80 border-slate-300 dark:border-slate-500 hover:border-rose-400 hover:bg-white'
                   }`}
                 >
                   {task.completed && <CheckCircle size={14} strokeWidth={3} />}
                 </button>
                 ...
                 <button onClick={() => deleteHomeworkTask(task.id)} className="opacity-0 group-hover:opacity-100 p-1 text-slate-400 hover:text-rose-500 transition-opacity"><Trash2 size={14} /></button>
               </div>
             ))}
          </div>

          <form onSubmit={handleAddHomework} className="relative mt-auto z-10">
             <input 
               type="text" value={newHomework} onChange={(e) => setNewHomework(e.target.value)} placeholder="Add homework assignment..."
               className="w-full bg-slate-50/80 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700/50 rounded-lg py-2 pl-3 pr-10 text-sm focus:outline-none focus:border-rose-500 dark:text-white placeholder-slate-400"
             />
             <button type="submit" className="absolute right-2 top-1/2 -translate-y-1/2 p-1 bg-rose-500 hover:bg-rose-600 text-white rounded-md transition-colors"><Plus size={14} /></button>
          </form>
             ...
             {nextFocusItems.length > 0 ? nextFocusItems.map((item, idx) => (
                 <div key={item.id} className="p-3 bg-slate-50/80 dark:bg-slate-800/40 rounded-xl border border-slate-100 dark:border-slate-700/50 hover:border-emerald-500/30 transition-all hover:translate-x-1 animate-slide-up" style={{ animationDelay: `${(idx + 1) * 100}ms` }}>
                    <div className="flex justify-between items-start mb-1">
                       <span className="text-[10px] font-bold text-emerald-600 dark:text-emerald-400 uppercase tracking-wider bg-emerald-50 dark:bg-emerald-900/20 px-1.5 py-0.5 rounded">Year {item.year}</span>
                       <span className="text-[10px] text-slate-400">{item.category}</span>
                    </div>
                    <h4 className="font-medium text-slate-800 dark:text-slate-200 text-sm line-clamp-2">{item.name}</h4>
                 </div>
               )) : <div className="h-full flex flex-col items-center justify-center text-slate-400 text-center py-6"><p className="text-sm">You're all caught up on high priority tasks!</p></div>}
          </div>
      ...
      {/* Motivation Quote */}
      <div className="bg-gradient-to-r from-emerald-600/90 to-teal-600/90 backdrop-blur-md rounded-2xl p-6 md:p-8 text-white relative overflow-hidden shadow-lg shadow-emerald-200/50 animate-slide-up border border-white/10" style={{ animationDelay: '300ms' }}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-2 animate-slide-up" style={{ animationDelay: '400ms' }}>
         <div className={`${glassCardClass}`}>
            <div className="flex items-center space-x-2 mb-4">
               <span className="p-1.5 bg-emerald-100/50 dark:bg-emerald-500/10 rounded-lg"><Quote className="text-emerald-600 dark:text-emerald-500" size={18} /></span>
               <h3 className="font-bold text-lg text-slate-900 dark:text-white">Real Advice</h3>
            </div>
            <ul className="space-y-3 text-sm text-slate-600 dark:text-slate-300">
               <li className="flex items-start"><span className="mr-2 text-emerald-600 dark:text-emerald-500">•</span>You don't need 50 courses. You need deeper execution.</li>
               <li className="flex items-start"><span className="mr-2 text-emerald-600 dark:text-emerald-400">•</span>Consistency &gt; Intelligence. Study 1-2 hours daily.</li>
               <li className="flex items-start"><span className="mr-2 text-emerald-600 dark:text-emerald-500">•</span>Communication skills are mandatory. Explain your projects clearly.</li>
            </ul>
         </div>
         <div className={`${glassCardClass}`}>
            <div className="flex items-center space-x-2 mb-4">
               <span className="p-1.5 bg-rose-100/50 dark:bg-rose-500/10 rounded-lg"><Flame className="text-rose-600 dark:text-rose-500" size={18} /></span>
