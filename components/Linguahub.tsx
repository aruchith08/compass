...
export const SKILL_DATA: SkillCategory[] = [
  ...
  },
  {
    ...
    resources: [
      {
        name: "British Council Reading",
        url: "https://learnenglish.britishcouncil.org/skills/reading",
      },
      {
        name: "IELTS Buddy Reading",
        url: "https://www.ieltsbuddy.com/ielts-reading.html",
      },
      {
  ...
  },
  {
    ...
    resources: [
      {
        name: "IELTS Liz Writing",
        url: "https://ieltsliz.com/ielts-writing-task-1-and-task-2/",
      },
  ...
  },
  {
    ...
    resources: [
      {
        name: "IELTS Liz Speaking",
        url: "https://ieltsliz.com/ielts-speaking-free-lessons-essential-tips/",
      },
      {
        name: "IELTS Speaking Samples",
        url: "https://ielts-up.com/speaking/ielts-speaking-samples.html",
      },
      {
        name: "TalkEnglish",
        url: "https://www.talkenglish.com/speaking/listbasics.aspx",
      },
    ],
  },
];
...
export const SAMPLE_PAPERS: ResourceLink[] = [
  ...
  {
    name: "Cambridge English Prep",
    url: "https://www.cambridgeenglish.org/learning-english/exam-preparation/",
  },
];
...
const sendMessageToGemini = async (
  ...
  return runGenAI(async (ai) => {
    ...
    // The history passed to chats.create must end with a model response if we are about to send a user message via sendMessage.
    // Or it can be empty.
    ...
    if (chatHistory.length > 0) {
      ...
      if (firstUserIndex !== -1) {
        for (let i = firstUserIndex; i < chatHistory.length; i++) {
          if (chatHistory[i].role === expectingRole) {
            sanitizedHistory.push(chatHistory[i]);
    ...
    // If the last message in sanitizedHistory is 'user', remove it, because `sendMessage` provides the next user turn.
    ...
    const chat = ai.chats.create({
      ...
      config: {
        systemInstruction:
          "You are an expert IELTS Tutor named LinguaBot. Your goal is to help students achieve Band 7.0+. Keep answers concise, professional, and focused on IELTS marking criteria (Lexical Resource, Grammatical Range, Coherence, Fluency).",
      },
    });
...
const checkVocabularyUsage = async (
  word: string,
  sentence: string
): Promise<string> => {
  try {
      ...
      >
        ...
        <div className="p-6 space-y-4">
          <p className="text-slate-600 dark:text-slate-300 mb-4 text-sm">
            Select a trusted source to practice your{" "}
            {skill.title.toLowerCase()} skills:
          </p>
          <div className="grid gap-3">
            {skill.resources.map((res, index) => (
              ...
              >
                ...
                <ExternalLink
                  size={18}
                  className="text-slate-400 dark:text-slate-500 group-hover:text-emerald-500 dark:group-hover:text-emerald-400"
                />
              </a>
            ))}
          </div>
...
const ChatWidget = () => {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      role: "model",
      text: "Hello! I'm LinguaBot, your IELTS Tutor. I can help with grammar, vocabulary, or simulate a speaking test. How can I help you today?",
    },
  ]);
      ...
      {isOpen && (
        <div className="mb-4 w-80 md:w-96 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl rounded-2xl shadow-2xl border border-slate-200/50 dark:border-white/10 overflow-hidden flex flex-col h-[500px] transition-all animate-fade-in">
          ...
          <form
            ...
            className="p-3 bg-white/50 dark:bg-slate-900/50 border-t border-slate-200/50 dark:border-white/10 flex gap-2 backdrop-blur-sm"
          >
            <input
              type="text"
              value={input}
...
const Linguahub: React.FC<{ user: User | null }> = ({ user }) => {
    ...
    const loadData = async () => {
      ...
      if (!sessionLoaded) {
        ...
        if (newTasks && newTasks.length > 0) {
          const newSession: DailySessionData = {
            date: today,
            tasks: newTasks,
            currentIndex: 0,
            isComplete: false,
  ...
  const updateSessionProgress = (newIndex: number, complete: boolean) => {
    ...
    const updatedSession: DailySessionData = {
      date: today,
      tasks: todayTasks,
  ...
  const handleCheckAnswer = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFeedback("Sorry, I couldn't check your answer right now.");
    } finally {
      setIsChecking(false);
    }
  };
  ...
  const glassCardClass =
    "bg-white/60 dark:bg-slate-900/60 backdrop-blur-md border border-white/20 dark:border-white/10 shadow-lg rounded-2xl";
    ...
    <div className="space-y-8 animate-fade-in pb-12">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400">
            Your IELTS AI Training Partner.
          </p>
        </div>
      </div>
      ...
      <section className="animate-in fade-in slide-in-from-top-4 duration-500 delay-100">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="text-lg md:text-xl font-bold text-slate-800 dark:text-slate-100 uppercase tracking-wider text-sm">
            Word of the Day
          </h2>
        </div>
        <div
          className={`${glassCardClass} p-6 relative overflow-hidden min-h-[250px] flex flex-col justify-center`}
        >
          {isVocabLoading ? (
              ...
              <div className="lg:col-span-2 space-y-4">
                <div>
                  <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1">
                    ...
                    <span className="px-2 py-0.5 rounded-full bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300 text-xs font-bold uppercase">
                      {vocabWord.partOfSpeech}
                    </span>
                  </div>
                  <p className="text-slate-700 dark:text-slate-300 mt-2 text-lg leading-relaxed">
                    {vocabWord.definition}
                  </p>
                </div>
                <div className="bg-slate-50/50 dark:bg-slate-800/50 p-4 rounded-xl border-l-4 border-purple-500">
                  <p className="italic text-slate-600 dark:text-slate-400 font-serif text-lg">
                    "{vocabWord.example}"
                  </p>
                </div>
              </div>
            ...
            <div className="text-center text-red-400">
              Failed to load vocabulary word.
            </div>
          )}
        </div>
      </section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          >
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400">
                Practice your spelling with Merriam-Webster's challenging word
                sets.
              </p>
            </div>
          ...
          >
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400">
                Look up advanced definitions, origins, and usage notes
                instantly.
              </p>
            </div>
            <div className="hidden sm:block">
      ...
      <section className="animate-in fade-in slide-in-from-top-4 duration-500">
        ...
        <div
          className={`${glassCardClass} p-4 md:p-8 transition-all min-h-[300px] flex flex-col justify-center relative`}
        >
          {isLoadingTasks ? (
            <div className="flex flex-col items-center justify-center py-12 gap-4 text-slate-500">
              <Loader2 size={40} className="animate-spin text-emerald-500" />
              <p className="animate-pulse">
                Consulting AI for today's IELTS tasks...
              </p>
            </div>
          ) : isSessionComplete ? (
            <div className="flex flex-col items-center justify-center py-8 md:py-12 gap-6 text-center animate-in zoom-in duration-300">
              ...
              <div>
                ...
                <p className="text-slate-600 dark:text-slate-400 max-w-md mx-auto">
                  Great job completing your daily IELTS challenges. Come back
                  tomorrow for a new set of tasks.
                </p>
              </div>
            </div>
          ) : todayTasks.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 text-center text-slate-500">
              ...
              <p className="mb-6 max-w-xs mx-auto">
                There was an issue connecting to the AI tutor. Please check your
                connection or quota.
              </p>
              <button
                ...
                {currentTask?.category === "Listening" &&
                  currentTask.hiddenContent && (
                    <button
                      onClick={() =>
                        currentTask.hiddenContent &&
                        playTextToSpeech(currentTask.hiddenContent)
                      }
                ...
                <div className="bg-slate-50/50 dark:bg-slate-800/50 p-4 rounded-xl border border-slate-100/50 dark:border-white/5 mt-4">
                  {!feedback ? (
                    ...
                    <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
                      <div
                        className={`p-4 rounded-lg border text-sm leading-relaxed mb-4 ${
                          lastScore !== null && lastScore >= 7
            ...
            >
              <div className="mb-4 bg-slate-50 dark:bg-slate-800 w-fit p-3 rounded-xl group-hover:bg-emerald-50 dark:group-hover:bg-emerald-900/30 transition-colors">
                {React.cloneElement(skill.icon as React.ReactElement<any>, {
                  className:
                    "w-8 h-8 group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors",
                })}
              </div>
