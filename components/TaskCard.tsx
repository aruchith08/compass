    ...
    <div 
      ...
      className={`
        ...
        ${item.status === 'Completed' 
          ...
          : 'bg-white/60 dark:bg-slate-900/60 border-white/20 dark:border-white/10 shadow-sm hover:shadow-lg hover:-translate-y-1 hover:bg-white/70 dark:hover:bg-slate-900/70'
        }
      `}
    >
      <div className="p-5 flex flex-col h-full">
        ...
        <div className="flex items-start justify-between gap-4 mb-3">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-2 flex-wrap">
              ...
              <span className="text-[10px] font-bold px-2 py-1 rounded-md bg-slate-100/50 dark:bg-slate-800/50 text-slate-500 dark:text-slate-400 uppercase tracking-wider truncate max-w-[150px]">
                {item.category}
              </span>
            </div>
        ...
        {/* Footer */}
        <div className="flex items-center justify-between pt-4 border-t border-slate-200/50 dark:border-white/5 mt-auto gap-4">
            <div className="flex gap-1.5 flex-wrap">
               {item.role_alignment.slice(0, 2).map((role, idx) => (
                 <span key={idx} className="text-[10px] px-2 py-1 rounded bg-slate-100/50 dark:bg-slate-800/50 text-slate-600 dark:text-slate-400 font-medium whitespace-nowrap">
                   {role === "All Roles" ? "Core" : role.split(" ")[0]}
                 </span>
               ))}
               {item.role_alignment.length > 2 && (
                 <span className="text-[10px] px-2 py-1 rounded bg-slate-100/50 dark:bg-slate-800/50 text-slate-500 dark:text-slate-500 font-medium">
                   +{item.role_alignment.length - 2}
                 </span>
               )}
            </div>
