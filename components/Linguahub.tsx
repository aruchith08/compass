...
const SkillModal = ({ isOpen, onClose, skill }: { isOpen: boolean, onClose: () => void, skill: SkillCategory | null }) => {
    ...
    return (
        ...
        >
        <div ref={modalRef} className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden transform transition-all scale-100 border border-slate-200 dark:border-slate-800">
            ...
            <p className="text-slate-600 dark:text-slate-300 mb-4 text-sm">Select a trusted source to practice your {skill.title.toLowerCase()} skills:</p>
            <div className="grid gap-3">
                {skill.resources.map((res, index) => (
                <a key={index} href={res.url} target="_blank" rel="noopener noreferrer" className="flex items-center justify-between p-4 rounded-xl border border-slate-200 dark:border-slate-800 bg-transparent hover:bg-emerald-50 dark:hover:bg-slate-800 hover:border-emerald-500 dark:hover:border-emerald-500 hover:shadow-md transition-all group">
...
const ChatWidget = () => {
    ...
    const [messages, setMessages] = useState<ChatMessage[]>([{ role: 'model', text: "Hello! I'm LinguaBot, your IELTS Tutor. I can help with grammar, vocabulary, or simulate a speaking test. How can I help you today?" }]);
    ...
    return (
        ...
        {isOpen && (
            ...
            <div className="bg-emerald-600/90 dark:bg-slate-950/90 backdrop-blur-md text-white p-4 flex justify-between items-center shadow-sm">
                <div className="flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-white/80 animate-pulse" /><h3 className="font-semibold">IELTS AI Tutor</h3></div>
                <button onClick={() => setIsOpen(false)} className="hover:bg-white/10 p-1 rounded-full transition"><X size={20} /></button>
            </div>
            <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50/50 dark:bg-slate-900/30">
                {messages.map((msg, idx) => (
                <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-[85%] p-3 rounded-2xl text-sm leading-relaxed shadow-sm ${msg.role === 'user' ? 'bg-emerald-600 text-white rounded-br-none' : msg.isError ? 'bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 border border-red-100 dark:border-red-800 rounded-bl-none' : 'bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-100 border border-slate-100 dark:border-slate-700 rounded-bl-none'}`}>{msg.isError && <AlertCircle size={16} className="inline mr-1 mb-0.5" />}{msg.text}</div>
                </div>
                ))}
                {isLoading && ( <div className="flex justify-start"><div className="bg-white dark:bg-slate-800 p-3 rounded-2xl rounded-bl-none shadow-sm border border-slate-100 dark:border-slate-700"><Loader2 className="w-5 h-5 text-emerald-500 animate-spin" /></div></div> )}
            ...
            <form onSubmit={handleSubmit} className="p-3 bg-white/50 dark:bg-slate-900/50 border-t border-slate-200/50 dark:border-white/10 flex gap-2 backdrop-blur-sm">
                <input type="text" value={input} onChange={(e) => setInput(e.target.value)} placeholder="Ask about IELTS..." className="flex-1 bg-slate-100 dark:bg-slate-800 dark:text-white border-0 rounded-xl px-4 py-2 text-sm focus:ring-2 focus:ring-emerald-500 outline-none transition-all placeholder-slate-400" />
                <button type="submit" disabled={isLoading || !input.trim()} className="bg-emerald-600 text-white p-2 rounded-xl hover:bg-emerald-700 disabled:opacity-50 transition-colors shadow-sm"><Send size={18} /></button>
            </form>
            </div>
        )}
        <button onClick={() => setIsOpen(!isOpen)} className="group flex items-center gap-2 bg-emerald-600 text-white px-5 py-4 rounded-full shadow-lg hover:bg-emerald-700 hover:scale-105 transition-all duration-300">
...
const Linguahub: React.FC<{ user: User | null }> = ({ user }) => {
        ...
        const loadData = async () => {
            ...
            if (savedSession) {
                try {
                    ...
                    if (session.tasks && Array.isArray(session.tasks) && session.tasks.length > 0) {
                        setTodayTasks(session.tasks); setCurrentTaskIndex(session.currentIndex); setIsSessionComplete(session.isComplete); setIsLoadingTasks(false); sessionLoaded = true;
                    }
            ...
            if (!sessionLoaded) {
                ...
                if (newTasks && newTasks.length > 0) {
                    const newSession = { date: today, tasks: newTasks, currentIndex: 0, isComplete: false };
                    localStorage.setItem(sessionKey, JSON.stringify(newSession));
                    setTodayTasks(newTasks); setCurrentTaskIndex(0); setIsSessionComplete(false);
                } else { setTodayTasks([]); }
    ...
    const updateSessionProgress = (newIndex: number, complete: boolean) => {
        ...
        const updatedSession = { date: today, tasks: todayTasks, currentIndex: newIndex, isComplete: complete };
    ...
    const handleCheckAnswer = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setFeedback(result.text); setLastScore(result.score);
        } catch (error) { setFeedback("Sorry, I couldn't check your answer right now."); } 
    ...
    const handleCheckVocab = async (e: React.FormEvent) => {
        ...
        setVocabFeedback(result); setIsVocabChecking(false);
    };

    const handleNextTask = () => {
        setUserAnswer(''); setFeedback(null); setLastScore(null);
        if (currentTaskIndex < todayTasks.length - 1) { updateSessionProgress(currentTaskIndex + 1, false); } 
        else { updateSessionProgress(currentTaskIndex, true); }
    };
    ...
    return (
        <div className="space-y-8 animate-fade-in pb-12">
                ...
                <div className={`${glassCardClass} p-6 relative overflow-hidden min-h-[250px] flex flex-col justify-center`}>
                    {isVocabLoading ? (
                        <div className="flex flex-col items-center justify-center py-8 gap-4 text-slate-500">
                             <Loader2 size={32} className="animate-spin text-purple-500" /><p>Curating today's advanced vocabulary...</p>
                        </div>
                    ) : vocabWord ? (
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                            ...
                            <div className="bg-slate-50/50 dark:bg-slate-800/30 rounded-xl p-5 border border-slate-100/50 dark:border-white/10 flex flex-col">
                                <h4 className="font-semibold text-slate-800 dark:text-slate-200 mb-3 flex items-center gap-2"><PenTool size={16} /> Try it yourself</h4>
                                {!vocabFeedback ? (
                                    <form onSubmit={handleCheckVocab} className="flex flex-col gap-3 flex-1">
                                        <textarea value={vocabInput} onChange={(e) => setVocabInput(e.target.value)} placeholder={`Write a sentence using "${vocabWord.word}"...`} className="w-full p-3 text-sm border border-slate-200 dark:border-slate-700 rounded-lg focus:ring-2 focus:ring-purple-500 outline-none bg-white/50 dark:bg-slate-900/50 text-slate-900 dark:text-white resize-none h-24" />
                                        <button type="submit" disabled={isVocabChecking || !vocabInput.trim()} className="mt-auto w-full bg-purple-600 hover:bg-purple-700 text-white py-2 rounded-lg font-medium transition-colors disabled:opacity-50 flex items-center justify-center gap-2">{isVocabChecking ? <Loader2 size={16} className="animate-spin" /> : "Check Usage"}</button>
                                    </form>
                                ) : (
                                    <div className="flex-1 flex flex-col animate-in fade-in slide-in-from-bottom-2">
                                        <div className="flex-1 text-sm text-slate-700 dark:text-slate-300 bg-white/50 dark:bg-slate-900/50 p-3 rounded-lg border border-slate-200 dark:border-slate-700 mb-3 overflow-y-auto max-h-[120px]">{vocabFeedback}</div>
                                        <button onClick={() => { setVocabFeedback(null); setVocabInput(''); }} className="w-full bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600 text-slate-800 dark:text-white py-2 rounded-lg font-medium transition-colors">Try Another Sentence</button>
                                    </div>
                                )}
                            </div>
                        </div>
            ...
            {/* Section: Language Power-Ups (Games & Tools) */}
            <section className="animate-in fade-in slide-in-from-top-4 duration-500 delay-150">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <a 
                        ...
                        className={`${glassCardClass} group p-6 flex items-center gap-6 hover:border-indigo-500/50 hover:shadow-xl transition-all border-l-4 border-l-indigo-500 active:scale-95`}
                    >
                        <div className="bg-indigo-100 dark:bg-indigo-900/40 p-4 rounded-2xl group-hover:scale-110 transition-transform">
                            <SpellCheck size={32} className="text-indigo-600 dark:text-indigo-400" />
                        </div>
                        <div className="flex-1">
                            <h3 className="text-lg font-bold text-slate-800 dark:text-white flex items-center gap-2">
                                Spelling Mastery (Games)
                                <ExternalLink size={14} className="opacity-0 group-hover:opacity-100 transition-opacity" />
                            </h3>
                            <p className="text-sm text-slate-500 dark:text-slate-400">Practice your spelling with Merriam-Webster's challenging word sets.</p>
                        </div>
                        <div className="hidden sm:block">
                            <ArrowRight size={24} className="text-slate-300 group-hover:text-indigo-500 group-hover:translate-x-1 transition-all" />
                        </div>
                    </a>
                    ...
                    <a 
                        ...
                        target="_blank" 
                        rel="noopener noreferrer"
                        className={`${glassCardClass} group p-6 flex items-center gap-6 hover:border-blue-500/50 hover:shadow-xl transition-all border-l-4 border-l-blue-500 active:scale-95`}
                    >
                        <div className="bg-blue-100 dark:bg-blue-900/40 p-4 rounded-2xl group-hover:scale-110 transition-transform">
                        ...
                        <div className="flex-1">
                            ...
                            <p className="text-sm text-slate-500 dark:text-slate-400">Look up advanced definitions, origins, and usage notes instantly.</p>
                        </div>
                        <div className="hidden sm:block">
                            <ArrowRight size={24} className="text-slate-300 group-hover:text-blue-500 group-hover:translate-x-1 transition-all" />
                        </div>
                    </a>
                </div>
            </section>
            ...
            <section className="animate-in fade-in slide-in-from-top-4 duration-500">
                ...
                {!isLoadingTasks && !isSessionComplete && todayTasks.length > 0 && (
                    <div className="text-sm font-medium px-3 py-1 bg-slate-200 dark:bg-slate-800 rounded-full text-slate-700 dark:text-slate-300">Question {currentTaskIndex + 1} of {todayTasks.length}</div>
                )}
                </div>
                ...
                {isLoadingTasks ? ( <div className="flex flex-col items-center justify-center py-12 gap-4 text-slate-500"><Loader2 size={40} className="animate-spin text-emerald-500" /><p className="animate-pulse">Consulting AI for today's IELTS tasks...</p></div> ) : isSessionComplete ? ( <div className="flex flex-col items-center justify-center py-8 md:py-12 gap-6 text-center animate-in zoom-in duration-300"><div className="bg-emerald-100 dark:bg-emerald-900/50 p-6 rounded-full"><CalendarCheck size={64} className="text-emerald-600 dark:text-emerald-400" /></div><div><h3 className="text-2xl md:text-3xl font-bold text-slate-800 dark:text-white mb-2">All Done For Today!</h3><p className="text-slate-600 dark:text-slate-400 max-w-md mx-auto">Great job completing your daily IELTS challenges. Come back tomorrow for a new set of tasks.</p></div></div> ) : todayTasks.length === 0 ? ( <div className="flex flex-col items-center justify-center py-12 text-center text-slate-500"><AlertCircle size={48} className="mb-4 text-red-400" /><h3 className="text-lg font-bold text-slate-700 dark:text-slate-300">Unable to generate tasks</h3><p className="mb-6 max-w-xs mx-auto">There was an issue connecting to the AI tutor. Please check your connection or quota.</p><button onClick={() => window.location.reload()} className="px-6 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 transition-colors shadow-lg hover:shadow-emerald-900/20">Retry Connection</button></div> ) : (
                    ...
                    <div className="flex-1 md:text-center"><span className="block text-xs font-bold uppercase tracking-wide text-slate-400">Type</span><span className="font-semibold text-slate-700 dark:text-slate-300 text-sm md:text-base">{currentTask?.category}</span></div>
                    ...
                    <div><span className="inline-block px-2 py-0.5 mb-2 bg-slate-100/50 dark:bg-slate-800/50 text-slate-500 dark:text-slate-400 rounded text-xs font-medium uppercase">{currentTask?.type}</span><h3 className="text-lg md:text-2xl font-serif text-slate-800 dark:text-slate-100 italic leading-relaxed">"{currentTask?.content}"</h3></div>
                    {currentTask?.category === 'Reading' && currentTask.hiddenContent && ( <div className="bg-slate-100/50 dark:bg-slate-800/50 p-4 rounded-xl border-l-4 border-blue-500 mb-4 shadow-sm"><div className="flex items-center gap-2 mb-2 text-blue-600 dark:text-blue-400"><BookOpen size={16} /><span className="text-xs font-bold uppercase">Reading Passage</span></div><p className="text-sm md:text-base leading-relaxed text-slate-700 dark:text-slate-300 font-serif whitespace-pre-line">{currentTask.hiddenContent}</p></div> )}
                    {currentTask?.category === 'Listening' && currentTask.hiddenContent && ( <button onClick={() => currentTask.hiddenContent && playTextToSpeech(currentTask.hiddenContent)} className={`w-full md:w-auto flex items-center justify-center gap-2 px-4 py-3 rounded-xl font-medium transition-all ${isPlayingAudio ? 'bg-amber-100 text-amber-700 animate-pulse' : 'bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-300 hover:bg-emerald-200'}`}>{isPlayingAudio ? <Loader2 size={18} className="animate-spin" /> : <PlayCircle size={18} />}{isPlayingAudio ? 'Playing Audio...' : 'Play Audio Clip'}</button> )}
                    <div className="bg-slate-50/50 dark:bg-slate-800/50 p-4 rounded-xl border border-slate-100/50 dark:border-white/5 mt-4">
                        {!feedback ? (
                            <form onSubmit={handleCheckAnswer} className="flex flex-col gap-3">
                                <label className="text-sm font-medium text-slate-600 dark:text-slate-400">{currentTask?.category === 'Speaking' ? 'Transcription (What you said):' : 'Your Answer:'}</label>
                                {currentTask?.requiresInput ? (
                                    <div className="flex flex-col gap-3">
                                        <textarea value={userAnswer} onChange={(e) => setUserAnswer(e.target.value)} placeholder="Type your response here..." className="w-full px-4 py-3 min-h-[100px] border border-slate-300/50 dark:border-slate-700/50 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none bg-white/50 dark:bg-slate-900/50 text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 resize-y text-base" />
                                        <button type="submit" disabled={isChecking || !userAnswer.trim()} className="w-full md:w-auto self-end bg-emerald-600 text-white px-6 py-3 rounded-lg hover:bg-emerald-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors font-medium flex items-center justify-center gap-2">{isChecking ? <Loader2 size={18} className="animate-spin" /> : <CheckCircle2 size={18} />}Check Answer</button>
                                    </div>
                                ) : ( <button onClick={handleNextTask} className="bg-emerald-600 text-white px-6 py-3 rounded-lg w-full">Mark as Done & Continue</button> )}
                            </form>
                        ) : (
                            <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
                                <div className={`p-4 rounded-lg border text-sm leading-relaxed mb-4 ${ (lastScore && lastScore >= 7) ? 'bg-emerald-50/50 dark:bg-emerald-900/20 border-emerald-100 dark:border-emerald-800 text-emerald-900 dark:text-emerald-200' : 'bg-amber-50/50 dark:bg-amber-900/20 border-amber-100 dark:border-amber-800 text-slate-800 dark:text-amber-100' }`}><div className="flex items-center justify-between gap-2 mb-2 border-b border-black/5 dark:border-white/5 pb-2"><strong className="font-semibold flex items-center gap-1">Examiner Feedback</strong>{lastScore !== null && ( <span className={`text-xs font-bold px-2 py-1 rounded-full ${ lastScore >= 7 ? 'bg-emerald-200 text-emerald-800' : 'bg-amber-200 text-amber-800' }`}>Band: {lastScore}/9</span> )}</div><div className="whitespace-pre-wrap text-base">{feedback}</div></div>
                                <button onClick={handleNextTask} className="w-full bg-slate-900 dark:bg-white text-white dark:text-slate-900 py-3 rounded-lg font-bold flex items-center justify-center gap-2 hover:opacity-90 transition-opacity">Next Task <ArrowRight size={18} /></button>
                            </div>
                        )}
                    </div>
                    </div>
            ...
            <section>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
                    ...
                    <button key={skill.id} onClick={() => setSelectedSkill(skill)} className={`${glassCardClass} p-5 md:p-6 hover:shadow-xl hover:border-emerald-200 dark:hover:border-emerald-700 hover:-translate-y-1 transition-all duration-300 text-left group h-full flex flex-col active:scale-95`}><div className="mb-4 bg-slate-50 dark:bg-slate-800 w-fit p-3 rounded-xl group-hover:bg-emerald-50 dark:group-hover:bg-emerald-900/30 transition-colors">{React.cloneElement(skill.icon as React.ReactElement<any>, { className: "w-8 h-8 group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors" })}</div><h3 className="text-lg font-bold text-slate-800 dark:text-slate-100 mb-2 group-hover:text-emerald-700 dark:group-hover:text-emerald-400">{skill.title}</h3><p className="text-slate-500 dark:text-slate-400 text-sm mb-4 flex-grow">{skill.description}</p><div className="flex items-center text-emerald-600 dark:text-emerald-400 text-sm font-medium mt-auto">View Resources <ArrowRight size={16} className="ml-1 group-hover:translate-x-1 transition-transform" /></div></button>
                    ))}
                </div>
            </section>
            ...
            <section className="bg-slate-900/60 dark:bg-slate-800/60 backdrop-blur-md rounded-3xl p-6 md:p-8 text-white shadow-xl relative overflow-hidden transition-colors duration-300 border border-white/10">
                ...
                <div className="relative z-10"><h2 className="text-xl md:text-2xl font-bold mb-2">Mock Tests & Sample Papers</h2><p className="text-slate-300 dark:text-slate-400 mb-6 max-w-xl text-sm md:text-base">Simulate exam conditions with free full-length tests from top providers.</p><div className="grid grid-cols-1 sm:grid-cols-3 gap-3 md:gap-4">{SAMPLE_PAPERS.map((paper, idx) => ( <a key={idx} href={paper.url} target="_blank" rel="noopener noreferrer" className="bg-white/10 backdrop-blur-sm border border-white/10 hover:bg-white hover:text-slate-900 hover:border-white px-4 py-3 md:px-5 md:py-4 rounded-xl transition-all font-medium flex items-center justify-between group text-sm md:text-base">{paper.name}<ExternalLink className="w-4 h-4 opacity-50 group-hover:opacity-100 transition-opacity" /></a> ))}</div></div>
            </section>
